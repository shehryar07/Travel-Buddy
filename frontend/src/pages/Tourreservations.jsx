  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        console.error("Error fetching tour reservations:", err);
        console.error("Error details:", err.response ? err.response.data : "No response data");
        console.error("Error status:", err.response ? err.response.status : "No status");
        
        setError(err.message || "Error fetching tour reservations");
        setLoading(false);
        Swal.fire({
          icon: "error",
  ...
  function generatePDF(reservations) {
    ...
    reservations
      ...
      .map((reservation, index) => {
        const resData = [
          index + 1,
          reservation.currentUser || "N/A",
          reservation.firstName || "N/A",
          reservation.lastName || "N/A",
    ...
    doc.text("Traverly Tour Reservation Report", 14, 15).setFontSize(12);
    doc.text(`Report Generated: ${dateStr}`, 14, 23);
    doc.save(`Tour-Reservation-Report_${dateStr}.pdf`);
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-row col-span-2 lg:px-32 px-8 pt-7 pb-2 justify-between md:items-center">
        <div className="text-3xl font-bold">Tour Reservations</div>
        <div>
          <button
