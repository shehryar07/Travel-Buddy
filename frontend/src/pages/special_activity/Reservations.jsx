...
function Reservation({ reservation, onCancel }) {
  ...
  const endTime = timeRange?.endTime || '17:00';
...
function ReservationPage() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/activity-reservations").then((response) => {
  ...
  const cancelReservation = (reservationId) => {
    ...
    axios.delete(`http://localhost:5000/api/activity-reservations/${reservationId}`).then(() => {
